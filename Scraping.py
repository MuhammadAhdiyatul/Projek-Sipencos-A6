...
def isi_kosong(data: dict) -> dict:
    """Isi semua nilai kosong dengan tanda -"""
    hasil = {}
...
def cari_setelah_label(soup: BeautifulSoup, label_regex: str) -> str:
...
def scrape_detail(item: dict) -> dict:
    ...
    alamat = cari_setelah_label(soup, r"^Alamat$")
    ...
    def fasilitas_setelah(label: str) -> list[str]:
    ...
    fasilitas_kamar    = fasilitas_setelah(r"Fasilitas Kamar")
    fasilitas_bersama  = fasilitas_setelah(r"Fasilitas Bersama")
    ...
    return isi_kosong(hasil)


def simpan(data: list[dict], filename: str = "data_kos.json") -> None:
    path = os.path.join(OUTPUT_DIR, filename)
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print(f"   Tersimpan: {path}")

def simpan_sqlite(data, db_file="output_dataKos/sipencos.db"):
...
def main():
    ...
    listing = ambil_listing()

    if not listing:
        print("Tidak ada kos ditemukan. Cek koneksi internet.")
        return

    simpan(listing, "data_kos_links.json")
    ...
    for idx, item in enumerate(listing, 1):
        ...
        if idx % 10 == 0:
            simpan(hasil, "data_kos_partial.json")

    simpan(hasil, "data_kos.json")
    simpan_sqlite(hasil)
    ...
    print("\nSelesai! Cek file: output_data/data_kos.json")
